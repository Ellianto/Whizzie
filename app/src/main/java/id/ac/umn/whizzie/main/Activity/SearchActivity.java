                ...
                for(DataSnapshot product : ds.getChildren()){
                    ...
                    dbRef.child("users").child(uid).addListenerForSingleValueEvent(new ValueEventListener() {
                        ...
                        public void onDataChange(@NonNull final DataSnapshot dataSnap) {
                            final String profPicPath = dataSnap.child("imgProfilePicture").getValue().toString();
                            final String profileName = dataSnap.child("toko").child("name").getValue().toString();
                            ...
                            dbRef.child("productRelation").child(itemKey).addListenerForSingleValueEvent(new ValueEventListener() {
                                ...
                                public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                                    ...
                                    SearchCardAdapter sca = new SearchCardAdapter(SearchActivity.this, scList);
rcSearch.setAdapter(sca);
    ...
    private void loadWishesCard(final String keyword){
        ...
        dbRef.child("wishes").addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot ds) {
                final long ssCount = ds.getChildrenCount();
                ...
                for(DataSnapshot wishes : ds.getChildren()){
                    ...
                    dbRef.child("users").child(uid).addListenerForSingleValueEvent(new ValueEventListener() {
                        ...
                        public void onDataChange(@NonNull DataSnapshot Snapshot) {
                            final String profPicPath = (Snapshot.child("imgProfilePicture").getValue() == null) ? "" : Snapshot.child("imgProfilePicture").getValue().toString();
                            final String profileName = Snapshot.child("name").getValue().toString();

                            // Fetch Offer Counts for this wishes
                            dbRef.child("wishRelation").child(itemKey).addListenerForSingleValueEvent(new ValueEventListener() {
                                @Override
                                public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                                    // Add to List
                                    scList.add(new SearchCard(
